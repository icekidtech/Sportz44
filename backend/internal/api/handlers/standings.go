...
// GetStandings handles GET /api/standings?league=<competitionID>
func (h *StandingsHandler) GetStandings(c *gin.Context) {
	...
	rows, err := h.standings.GetStandings(c.Request.Context(), uint(compID))
	if err != nil {
...
// GetTopScorers handles GET /api/standings/:league/top-scorers
func (h *StandingsHandler) GetTopScorers(c *gin.Context) {
	...
	rows, err := h.standings.GetTopScorers(c.Request.Context(), uint(compID), limit)
	if err != nil {
